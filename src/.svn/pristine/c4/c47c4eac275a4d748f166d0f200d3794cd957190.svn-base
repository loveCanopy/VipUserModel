	...
	public static void main(String[] args) {
		...
		try {
			...
			if (!lists.isEmpty()) {
				for (Date date : lists) {
					...
					if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY
						...
						String insertSql = "INSERT INTO fn_all_holiday (id,title,holiday_date) VALUES('"
								+ UUID.randomUUID() + "'," + "'周末','" + sdf.format(date) + "')";
			...
			for (Days day : holidays) {
				// 跟周末冲突的，不重复插入
				String sql = "select count(1) as numbers from fn_all_holiday where holiday_date ='"
				...
				if (!hasRecord) {
					...
					String insertSql = "INSERT INTO fn_all_holiday (id,title,holiday_date) VALUES('" + day.getId()
							+ "'," + "'" + day.getTitle() + "','" + sdf.format(day.getDate()) + "')";
					statement.executeUpdate(insertSql);
				}
			}
